...
/******************************************************************************/
uint32 PROZ_NP3::CheckModule(const PBinary &module)
{
	const uint8 *mod;
	uint32 offset;
	uint16 temp;
	...
	for (i = 0; i < sampNum; i++)
	{
		...
		temp1 = P_BENDIAN_TO_HOST_INT32(*((uint32 *)&mod[8 + i * 16 + 2]));
		if (temp1 > 0xfe000)
			return (0);
		...
		if ((mod[8 + i * 16 + 1] > 0x40) || (mod[8 + i * 16] > 0x0f))
			return (0);

		// Check sample size
		temp = P_BENDIAN_TO_HOST_INT16(*((uint16 *)&mod[8 + i * 16 + 6]));
		if (temp == 0)
			return (0);
...
/******************************************************************************/
ap_result PROZ_NP3::ConvertModule(const PBinary &module, PFile *destFile)
{
	const uint8 *mod;
	uint8 zeroBuf[128];
	uint16 pattern[512];
	...
	uint16 temp;
	uint16 i, j, k;
	const uint8 *addyPos, *addyTracks, *addyPatt, *addySamples;
	const uint8 *notePoi;
	...
	for (i = 0; i < sampNum; i++)
	{
		...
		// Sample size
		destFile->Write(&mod[8 + i * 16 + 6], 2);

		// finetune + volume
		destFile->Write(&mod[8 + i * 16], 2);

		// Repeat point
		destFile->Write(&mod[8 + i * 16 + 14], 2);
	...
	posNum = P_BENDIAN_TO_HOST_INT16(*((uint16 *)&mod[2])) / 2;
	...
	addyTracks  = addyPos + posNum * 2;
	addyPatt    = addyTracks + P_BENDIAN_TO_HOST_INT16(*((uint16 *)&mod[4]));
	addySamples = addyPatt + P_BENDIAN_TO_HOST_INT16(*((uint16 *)&mod[6]));
	...
	trackNum = P_BENDIAN_TO_HOST_INT16(*((uint16 *)&mod[4])) / 2;
	...
	for (i = 0; i < pattNum; i++)
	{
		...
		for (j = 0; j < 4; j++)
		{
			...
			notePoi = &addyPatt[P_BENDIAN_TO_HOST_INT16(*((uint16 *)addyTracks))];
			addyTracks += 2;
...
/******************************************************************************/
int32 PROZ_NP3::GetModuleType(void)
{
	return (IDS_PROZ_TYPE_NP3);
}
...
/******************************************************************************/
void PROZ_NP3::BuildVal1(uint8 dat1)
{
	uint8 note;
...
/******************************************************************************/
void PROZ_NP3::BuildVal2(uint8 dat2, uint8 dat3)
{
