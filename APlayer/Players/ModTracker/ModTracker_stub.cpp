...
void Load(APGlobalData *global, PString fileName)
{
	...
	// Get the MOD file type strings
	type.LoadString(&res, IDS_MOD_MIME);
	ext.LoadString(&res, IDS_MOD_EXTENSION);
	shortDescr.LoadString(&res, IDS_MOD_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_MOD_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_MOD_FILEICON, type, ext, longDescr, shortDescr);

	// Get the MTM file type strings
	type.LoadString(&res, IDS_MTM_MIME);
	ext.LoadString(&res, IDS_MTM_EXTENSION);
	shortDescr.LoadString(&res, IDS_MTM_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_MTM_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_MTM_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new ModTracker(global, fileName));
}
