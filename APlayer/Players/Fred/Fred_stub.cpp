...
void Load(APGlobalData *global, PString fileName)
{
	...
	type.LoadString(&res, IDS_FRED_MIME);
	ext.LoadString(&res, IDS_FRED_EXTENSION);
	shortDescr.LoadString(&res, IDS_FRED_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_FRED_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_FRED_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new Fred(global, fileName));
}
