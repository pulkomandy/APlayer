...
void Load(APGlobalData *global, PString fileName)
{
	...
	ext.LoadString(&res, IDS_TFMX_EXTENSION);
	shortDescr.LoadString(&res, IDS_TFMX_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_TFMX_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_TFMX_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new TFMX(global, fileName));
}
