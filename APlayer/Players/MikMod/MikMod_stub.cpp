...
void Load(APGlobalData *global, PString fileName)
{
	...
	ext.LoadString(&res, IDS_MIK_EXTENSION);
	shortDescr.LoadString(&res, IDS_MIK_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_MIK_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_MIK_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new MikMod(global, fileName));
}
