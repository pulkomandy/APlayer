...
void Load(APGlobalData *global, PString fileName)
{
	...
	ext.LoadString(&res, IDS_OGG_EXTENSION);
	shortDescr.LoadString(&res, IDS_OGG_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_OGG_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_OGG_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new OggVorbis(global, fileName));
}
