...
void Load(APGlobalData *global, PString fileName)
{
	...
	ext.LoadString(&res, IDS_AHX_EXTENSION);
	shortDescr.LoadString(&res, IDS_AHX_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_AHX_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_AHX_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new AHX(global, fileName));
}
