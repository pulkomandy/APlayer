...
void Load(APGlobalData *global, PString fileName)
{
	...
	// Get the MED file type strings
	type.LoadString(&res, IDS_MED_MIME);
	ext.LoadString(&res, IDS_MED_EXTENSION);
	shortDescr.LoadString(&res, IDS_MED_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_MED_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_MED_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new OctaMED(global, fileName));
}
