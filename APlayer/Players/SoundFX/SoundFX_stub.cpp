...
void Load(APGlobalData *global, PString fileName)
{
	...
	ext.LoadString(&res, IDS_SFX_EXTENSION);
	shortDescr.LoadString(&res, IDS_SFX_SHORT_DESCRIPTION);
	longDescr.LoadString(&res, IDS_SFX_LONG_DESCRIPTION);

	// Install the mime type + icon
	global->fileTypes->RegisterFileType(&res, IDI_SFX_FILEICON, type, ext, longDescr, shortDescr);
}
...
{
	return (new SoundFX(global, fileName));
}
