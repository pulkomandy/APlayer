/******************************************************************************/
/* APToolTips implementation file.                                            */
/******************************************************************************/
...
/* Default constructor                                                        */
/******************************************************************************/
APToolView::APToolView(void) : BView(BRect(0.0f, 0.0f, 10.0f, 10.0f), NULL, B_FOLLOW_ALL, B_WILL_DRAW)
{
}
...
/******************************************************************************/
void APToolView::SetText(PString text)
{
	tipText = text;
}
...
/******************************************************************************/
void APToolView::Draw(BRect updateRect)
{
	BRect rect;
	font_height fh;
	float pos, height;
...
/* Constructor                                                                */
/******************************************************************************/
APToolTips::APToolTips(BRect frame) : BView(frame, NULL, B_FOLLOW_ALL_SIDES, B_WILL_DRAW)
{
	...
	tipWin = new BWindow(BRect(0.0f, 0.0f, 10.0f, 10.0f), "MainWindowSystem: Tooltip window", B_NO_BORDER_WINDOW_LOOK, B_FLOATING_APP_WINDOW_FEEL, B_AVOID_FRONT | B_AVOID_FOCUS | B_NO_WORKSPACE_ACTIVATION | B_ASYNCHRONOUS_CONTROLS);
	if (tipWin == NULL)
		throw PMemoryException();

	// Create the view used to draw the tip
	tipView = new APToolView();
	if (tipView == NULL)
	{
		delete tipWin;
		throw PMemoryException();
	...
	tipThread.SetName("MainWindowSystem: Tooltip handling");
...
/******************************************************************************/
void *APToolTips::AddToolTip(BView *view, PString text)
{
	APViewData *data;
...
/* Output: A pointer to the item added. This can be used to remove it later.  */
/******************************************************************************/
void *APToolTips::AddToolTip(BView *view, PResource *resource, int32 strNum)
{
	PString text;
...
/******************************************************************************/
/* SetEnabled() will enable or disable the tool tips.                         */
/*                                                                            */
/* Input:  "enable" is true if you want to enable it, false if not.           */
/******************************************************************************/
void APToolTips::SetEnabled(bool enable)
{
	enabled = enable;
}
...
/* Output: Always 0.                                                          */
/******************************************************************************/
int32 APToolTips::TipThread(void *userData)
{
	APToolTips *object = (APToolTips *)userData;
	BWindow *win;
...
void APToolTips::ShowTip(void)
{
	...
	if (timeCounter == 4)
	{
		if ((!showing) && (!notAgain) && (enabled) && (Window()->IsActive()))
		{
			BScreen scr(B_MAIN_SCREEN_ID);
			BRect scrRect;
			PString tempStr;
			char *tempPtr;
			APViewData *viewData = NULL;
			font_height fh;
